...
class PowerModel:
    ...
    def __init__(self,model_type,model_size,learning_rate = 0.001,batch_size=16,
        ...
        if(model_type == "lstm"):
            ...
            self.fused_cell = NODE(model_size,cell_clip=10,W_in_mask=self._W_in_mask)
            head,_ = tf.nn.dynamic_rnn(self.fused_cell,head,dtype=tf.float32,time_major=True)
        ...
        self.y = tf.layers.Dense(1, activation=None, name="dense_readout",
            kernel_initializer=tf.keras.initializers.TruncatedNormal())(masked_state)
        self.y = tf.squeeze(self.y, axis=-1)  # (batch,)
    ...
    def fit(self,power_data,epochs,verbose=True,log_period=50):
        ...
        for e in range(epochs):
            ...
            if(e > 0 and e % 10 == 0):
                self.save_named("_epochpower_data".format(e))
                ...
                run_lyapunov_if_due(
                    ...
                    power_data.valid_x, os.path.join("lyapunov", "power"),
